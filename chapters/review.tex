...
\section{Методы на основе SSFM (метод расщепления Фурье)}
...
Затем обучение проводится путём минимизации функции потерь $L_S(\theta)$ на обучающей выборке, которая определяется как~\cite{nielsen2015neural}:

\begin{equation}
	L_S(\theta) = \frac{1}{|A|} \sum \limits_{(x, y) \in S} l(f_{\theta}(x), y),
\end{equation}
